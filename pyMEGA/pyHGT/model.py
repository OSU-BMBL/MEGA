...
class Classifier(nn.Module):
    ...
    def forward(self, x):
        tx = self.linear(x)
        return torch.log_softmax(tx.squeeze(), dim=-1), torch.softmax(
            tx.squeeze(), dim=-1
        )
    ...
    def forward(self, node_feature, node_type, edge_time, edge_index, edge_type):
        ...
        for gc in self.gcs:
            ...
            if self.conv_name == "hgt":
                # self.att = gc.res_att
                self.att1[i] = gc.res_att1
                self.att2[i] = gc.res_att2
                # self.emb[i]=gc.res
                i = i + 1
                # print(gc.res_att)
                ...
                #    print(p)
        # self.att = gc.res_att
        # 根据gnn层数修改，2层为1，1层需要改成0
        self.att1 = self.att1[1]
        self.att2 = self.att2[1]
        return meta_xs
